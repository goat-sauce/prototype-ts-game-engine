...
export class GameObject<S> {
    ...
    public async inject(object: DisplayObject): Promise<void> {
        Engine.stage.container.addChild(object)
    }

    public register(task: GameObject<S>, render: DisplayObject): void {
        Engine.registry.set(task.ref, { task, render })
    }

    public destroy(): void {
        Engine.registry.delete(this.ref)
    }
}
